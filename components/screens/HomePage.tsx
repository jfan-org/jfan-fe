				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="text-center mb-16">
						...
						<p className="text-xl text-gray-300 max-w-3xl mx-auto">
							The Job-seekers and Freelancers Africa Network (JFAN) is a comprehensive platform designed to
							connect Africa's vast talent pool with global opportunities while fostering professional
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					...
					<div className="grid lg:grid-cols-3 gap-8">
						{jobCatalog.economies.map((economy: Economy) => (
							<Card key={economy.id}>
								<h3 className="text-2xl font-bold mb-4 text-green-400">{economy.name}</h3>
								<div className="space-y-3">
									{jobCatalog.industries
										.filter((industry: Industry) => industry.economyId === economy.id)
										.slice(0, 3)
										.map((industry: Industry) => (
											<div key={industry.id} className="p-3 bg-gray-900 rounded-lg">
												<span className="text-gray-300">{industry.name}</span>
											</div>
										))}
								</div>
								<Button className="w-full mt-6" href="/jobs">
